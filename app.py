...
def main():
    ...
    # Set default to 'Upload' (index 1) to avoid webcam crash on start in cloud env
    input_source = st.sidebar.radio("Input Source", ["Webcam", "Upload Image/Video"], index=1)
    ...
    # Initialize Core Engine (Unified YOLO-Pose) - LOAD ONCE via Cache
    ...
    if input_source == "Webcam":
        ...
        if not cap.isOpened():
            st.error("❌ Webcam could not be accessed. (Note: Cloud hosting does not support local webcams).")
            cap = None
...
@st.cache_resource
def load_yolo_model():
    """Load model once and keep in memory to save RAM on Streamlit Cloud."""
    return YoloHandler(model_path='yolo11s-pose.pt', conf=0.2)
